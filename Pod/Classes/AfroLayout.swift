    ...
    public func stackViews(
        ...
        for index in 0...(stackedViews.count - 1) {
            if let nLastView = lastView {
                ...
                stackedViews[index].addCustomConstraints(inView: self, toViews: [nLastView, self, self], selfAttributes: [NSLayoutAttribute.top] + hAttributes[index], relations: [NSLayoutRelation.equal] + hRelations[index], otherViewAttributes: [NSLayoutAttribute.bottom] + hAttributes[index], padding: [gapPadding] + hPadding[index])
                
            } else {
                stackedViews[index].addCustomConstraints(inView: self, toViews: topLayoutGuide == nil ? nil : [topLayoutGuide!, self, self], selfAttributes: [NSLayoutAttribute.top] + hAttributes[index], relations: [NSLayoutRelation.equal] + hRelations[index], otherViewAttributes: (topLayoutGuide == nil ?  [NSLayoutAttribute.top] : [NSLayoutAttribute.bottom]) + hAttributes[index], padding: [topPadding] + hPadding[index])
            }
            lastView = stackedViews[index]
        }
        
        if let nLastView = lastView {
            nLastView.addCustomConstraints(inView: self, selfAttributes: [.bottom], relations: [bottomRelation], otherViewAttributes: [.bottom], padding: [-bottomPadding])
        }
	}
    ...
    public func stackHorizontallViews(
        ...
        for index in 0...(stackedViews.count - 1) {
            if let nLastView = lastView {
                ...
                stackedViews[index].addCustomConstraints(inView: self, toViews: [nLastView, self, self], selfAttributes: [NSLayoutAttribute.left] + vAttributes[index], relations: [NSLayoutRelation.equal] + vRelations[index], otherViewAttributes: [NSLayoutAttribute.right] + vAttributes[index], padding: [gapPadding] + vPadding[index])
                
            } else {
                stackedViews[index].addCustomConstraints(inView: self, selfAttributes: [NSLayoutAttribute.left] + vAttributes[index], relations: [NSLayoutRelation.equal] + vRelations[index], otherViewAttributes:  [NSLayoutAttribute.left] + vAttributes[index], padding: [leftPadding] + vPadding[index])
            }
            lastView = stackedViews[index]
        }
        
        if let nLastView = lastView {
            nLastView.addCustomConstraints(inView: self, selfAttributes: [.right], relations: [rightRelation], otherViewAttributes: [.right], padding: [-rightPadding])
        }
    }
